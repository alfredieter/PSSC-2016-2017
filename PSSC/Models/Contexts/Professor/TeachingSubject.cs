...
namespace Models.Contexts.Professor
{
    public class TeachingSubject : Subject
    {
        ...
        public void ModifyExamGrade(RegistrationNumber regNumber, Grade grade, int index)
        {
            for (int i = 0; i < _enrolledStudents.Count(); i++)
            {
                if (_enrolledStudents.ElementAt(i).Key.RegNumber == regNumber)
                {
                    _enrolledStudents[_enrolledStudents.ElementAt(i).Key].ExamGrades[index] = grade;
                }      
            }
        }
